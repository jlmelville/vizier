...
For categories, a fully named color vector is a value-to-color mapping. This is
useful when subsets or reordered factor levels should retain the same colors:
...
Extra names are ignored, but every observed category must have a color. An
unnamed vector remains a positional palette. The `rev` argument reverses a
generated or named category palette before values are mapped; it does not
reorder explicit per-observation colors supplied with `colors`.
