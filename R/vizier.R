...
#' @param top If not `NULL`, only this many finite values from a directly
#'   supplied numeric `x` are displayed. Values are selected in decreasing order
...
embed_plot <- function(
  ...
  if (!is.null(text)) {
    graphics::plot(
      ...
      xlim = coord_res$xlim,
      ylim = coord_res$ylim,
      asp = if (coord_res$fixed_aspect) 1 else NA,
      xlab = xlab,
      ylab = ylab,
      main = title,
      axes = axes,
