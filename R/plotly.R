...
#' @param text Vector of label text to display instead of a point, including for
#'   numeric color scales. If the labels are long or the data set is large, this
#'   is unlikely to be very legible, but is occasionally useful.
#' @param tooltip Vector of tooltip text, to be displayed when a point is
#'   hovered over. Without it, hover text uses `text`, then resolved labels, and
#'   always retains the original row-number prefix. Explicit colors alone are
#'   not used as hover labels.
#' @param title Title for the plot.
#' @param show_legend If `TRUE`, display a categorical legend or numeric
#'   colorbar when a suitable value is provided as `x` (or one can be found).
#' @param equal_axes If `TRUE`, use a common coordinate range and equal physical
...
#'   X-axis after any PCA rotation. Ignored if `equal_axes = TRUE`.
#' @param ylim Vector of two numeric values to give the numeric extent of the
#'   Y-axis after any PCA rotation. Ignored if `equal_axes = TRUE`.
#' @param rev Logical indicating whether generated palettes or continuous scales
  ...
  p <-
    plotly::layout(
      ...
      xaxis = list(
        ...
        showline = TRUE,
        showgrid = FALSE,
        range = coord_res$xlim,
        scaleanchor = if (coord_res$fixed_aspect) "y" else NULL,
        scaleratio = if (coord_res$fixed_aspect) 1 else NULL
      ),
      yaxis = list(
        title = "Y",
        zeroline = FALSE,
