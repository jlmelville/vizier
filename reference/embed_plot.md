...
- top:
  ...
  If not `NULL`, only the specified number of points will be displayed,
  corresponding to those with the highest values in `x`, after sorting
  by decreasing order.
...
- The name of a pre-defined R palette, if you are running R 4.0 or
  ...
  `"Alphabet"`. See
  [`palette.pals`](https://rdrr.io/r/grDevices/palette.html) for the
  list of possible names. Note that is function is not available prior
  to R 4.0. For more details, see
