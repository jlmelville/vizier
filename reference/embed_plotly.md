...
- show_legend:
  ...
  If `TRUE`, display a categorical legend or numeric colorbar when a
  suitable value is provided as `x` (or one can be found).
...
- text:
  ...
  Vector of label text to display instead of a point, including for
  numeric color scales. If the labels are long or the data set is large,
  this is unlikely to be very legible, but is occasionally useful.

- tooltip:

  Vector of tooltip text, to be displayed when a point is hovered over.
  Without it, hover text uses `text`, then resolved labels, and always
  retains the original row-number prefix. Explicit colors alone are not
  used as hover labels.
